// column { string name, bool unique, bool index  }
...
var nextHandleItemId = 1;

class Column {    
    constructor(id, name, unique, index) {
        this.id = id;
        this.name = name;
        this.unique = unique;
        this.index = index;
...
export function createDb() {    
    ...
    var name = "Car";
    var makeColumn = new Column(nextHandleItemId++, "Make", false, true);
    var modelColumn = new Column(nextHandleItemId++, "Model", false, true);
    var colorColumn = new Column(nextHandleItemId++, "Color", false, true);
    var yearColumn = new Column(nextHandleItemId++, "Year", false, true);
    var columns = { makeColumn, modelColumn, colorColumn, yearColumn };

    // open the db - 1 is the version of the db, needs to be incremented if structure (schema) is changed
    const request = indexedDB.open(name + "Database", 1);
    ...
    // listen for and handle upgradeneeded event - triggered when the database version is than the existing db in the client browser or a new db is being created
    request.onupgradeneeded = function () {
        ...
        for (const column of columns {  
            if (column.index) {
                store.createIndex(makeColumn.name + "Index", [column.name], { unique: column.unique });
            }
        }
